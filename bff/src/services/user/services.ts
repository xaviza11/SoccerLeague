...
export class UserServices {
  ...
  private playersClient = new PlayersClient();
  ...
  public async registerUser(payload: ServiceUserRegistrationPayload) {
    try {
      const user = await this.usersClient.create(payload);
      ...
      const login = await this.usersClient.login({
        email: payload.email,
      ...
      if (this.currentPassword && this.token) {
        await this.usersClient.deleteOne(this.token, {
  ...
  public async login(payload: ServiceUserLoginPayload) {
    const user = await this.usersClient.login(payload);
