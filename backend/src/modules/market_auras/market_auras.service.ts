...
export class MarketAurasService {
  ...
  async create(payload: {
    ...
    this.logger.log(`Creating market aura with payload: ${JSON.stringify(payload)}`);
    const { aura_id, seller_id, price } = payload;

    if (price < 0) {
      this.logger.log(`Attempted to create market aura with negative price: ${price}`);
  ...
  async updatePrice(id: string, price: number, userId: string): Promise<MarketAura> {
    this.logger.log(`Updating price for market aura with id: ${id} to ${price}`);
    if (price < 0) {
      this.logger.log(`Attempted to update market aura with negative price: ${price}`);
      throw new BadRequestException("Price must be positive");
    }
