...
export class MarketPositionChangeCardsService {
  ...
  async create(payload: { position_change_card_id: string; price: number; seller_id: string }): Promise<MarketPositionChangeCard> {
    ...
    this.logger.log(`Market position change card entry created successfully with ID: ${response.id}`);
    return response;
  }

  async findAll(): Promise<MarketPositionChangeCard[]> {
    this.logger.log(`Fetching all market position change card entries`);
    const response = await this.marketCardsRepo.find({ relations: ['positionChangeCard'], order: { createdAt: 'DESC' } });
  ...
  async findBySeller(seller_id: string): Promise<MarketPositionChangeCard[]> {
    ...
    const response = await this.marketCardsRepo.find({ where: { seller_id }, relations: ['positionChangeCard'], order: { createdAt: 'DESC' } });
    this.logger.log(`Market position change card entries for seller ID ${seller_id} retrieved successfully`);
    return response;
  }
  ...
  async updatePrice(id: string, price: number, userId: string): Promise<MarketPositionChangeCard> {
    ...
    if (record.seller_id !== userId) {
      this.logger.error(`User with ID ${userId} is not the seller of market position change card entry with ID ${id}`);
