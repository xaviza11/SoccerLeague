...
export class PositionChangeCardsService {
  ...
  async create(storage: string, userId: string) {
    this.logger.log(
      `Creating position change card for user ID: ${userId} with storage ID: ${storage}`,
    );
  ...
  async findOne(id: string, userId: string) {
    ...
    if (!card) {
      ...
      throw new NotFoundException("Card not found or does not belong to this user");
    }

    this.logger.log(
      `Position change card retrieved successfully with ID: ${id} for user ID: ${userId}`,
    );
    return card;
  }
