...
describe('TeamsController', () => {
  ...
  it('should return the logged user team on getMyTeam', async () => {
    const team = { id: 'team-123' };

    mockTeamsService.findOne.mockResolvedValue(team);

    const result = await controller.getMyTeam(mockUserReq);

    expect(service.findOne).toHaveBeenCalledWith('team-123');
    expect(result).toEqual(team);
  });
  ...
  it('should delete the logged user team on deleteMyTeam', async () => {
    const deleted = { message: 'Team deleted successfully' };

    mockTeamsService.delete.mockResolvedValue(deleted);

    const result = await controller.deleteMyTeam(mockUserReq);

    expect(service.delete).toHaveBeenCalledWith('team-123');
